...
-- modified by xuyaoqiang

local stringy = require("guan.utils.stringy")
...
function _M.os_execute(command, preserve_output)
    ...
    os.remove(n)
    os.remove(f)
    return preserve_output and result or string.gsub(
        string.gsub(result, "^"..f..":[%s%w]+:%s*", ""), "[%\r%\n]", ""), exit_code / 256
end
...
function _M.kill_process_by_pid_file(pid_file, signal)
    if _M.file_exists(pid_file) then
        ...
        return _M.os_execute(
                "while kill -0 "..pid.." >/dev/null 2>&1; do kill "..(
                    signal and "-"..tostring(signal).." "
                    or "")..pid.."; sleep 0.1; done")
    end
end
